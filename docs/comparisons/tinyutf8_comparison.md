...
# Comparison: tinyutf8 and SuiteUTF UTF-8 handling

## Scope and intent

This document compares tinyutf8 and SuiteUTF at the level of UTF-8 encoding and
decoding behavior.
...
SuiteUTF behavior is described according to its documented guarantees and intended
semantics.
...
SuiteUTF is a lower-level, allocation-free library focused on explicit UTF encode
...
normalization, while SuiteUTF emphasizes explicit policy selection and observable
behavior.
...
SuiteUTF core encode and decode operations perform no allocation. All buffers,
...
SuiteUTF intentionally caps UTF-8 encoding at 1 to 6 bytes. This allows handling
...
In SuiteUTF, attempting to encode a UTF-8 sequence longer than 6 bytes is a
write-side error and reports BadSizeUTF8.

During decoding, SuiteUTF always treats lead bytes 0xFE and 0xFF as illegal input
...
SuiteUTF distinguishes overlong encodings explicitly. Overlong UTF-8 sequences can
...
SuiteUTF also provides a controlled mechanism for treating overlong encodings as
...
SuiteUTF validates UTF-8 structure in both strict and toolkit modes. If a byte is
...
SuiteUTF also records the relative offset of the offending byte within the
attempted decode using reserved diagnostic bits. SuiteUTF does not attempt to
...
SuiteUTF treats BOM handling explicitly. BOMs may be emitted deliberately, and
...
## Rationale for the SuiteUTF 6-byte UTF-8 cap

SuiteUTF caps extended UTF-8 at 6 bytes as a deliberate policy choice.
...
This cap aligns with SuiteUTF goals of controlled permissiveness. Legacy and
